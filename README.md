#  Hi, I’m [zoomlogo](https://codeberg.org/zoomlogo "Codeberg account.")

### Stuff I like and/or use.
[APL](https://tryapl.org/ "Best Array Language."), [C/C++](https://cppreference.com "C > C++."), [Iosevka](https://typeof.net/Iosevka/ "Favourite font."), [Jelly](https://github.com/DennisMitchell/jellylanguage "Best code-golfing language there is."),
[Mathematics](https://en.wikipedia.org/wiki/Mathematics "Who doesn't like mathematics?"), [Physics](https://en.wikipedia.org/wiki/Physics "...or Physics for that matter?"), [ngn/k](https://codeberg.org/growler/k "Favourite Programming Language."), [Python](https://www.python.org/ "Quick and slow... at the same time."), [Rust](https://rust-lang.org/ "Holy borrow semantics."), [Vim](https://www.vim.org/ "The BEST editor. No one needs anything else (except Emacs)."), [Vyxal](https://github.com/Vyxal/Vyxal "Cookie?"),
Anything and everything low-level and/or related to electronics.

### Don't quote me on this.

> One day I shall complete [flax](https://github.com/zoomlogo/flax).
> - zoomlogo, 2022.

> Hopefully, one day...
> - zoomlogo, 2025.

### Am I a [[BIG SHOT]]?

![](https://github-readme-stats.vercel.app/api/top-langs/?username=zoomlogo&layout=compact&theme=gruvbox)

### Potato Stats 👑
![](https://github-readme-stats.vercel.app/api?username=zoomlogo&theme=gruvbox)
